...
var Diagnostics = (function () {
    ...
    function ensureItemDiagnostics() {
        try {
            ...
            appInsights.trackEvent("diagError", { source: "Diagnostics.ensureItemDiagnostics", exception: e.toString(), message: e.message, stack: e.stack });
        }
    }
    function ensureOfficeDiagnostics() {
        try {
            ...
            appInsights.trackEvent("diagError", { source: "Diagnostics.ensureOfficeDiagnostics", exception: e.toString(), message: e.message, stack: e.stack });
        }
    }
    function getRequirementSet() {
        ...
        try {
            ...
            if (window.Office.context.requirements && window.Office.context.requirements.isSetSupported) {
                if (window.Office.context.requirements.isSetSupported("Mailbox", 1.9))
                    return "1.9";
                if (window.Office.context.requirements.isSetSupported("Mailbox", 1.8))
                    return "1.8";
            ...
            if (window.Office.context.mailbox && window.Office.context.mailbox.addHandlerAsync)
                return "1.5?";
            if (window.Office.context.ui && window.Office.context.ui.displayDialogAsync)
                return "1.4?";
            if (window.Office.context.mailbox && window.Office.context.mailbox.item.saveAsync)
                return "1.3?";
            if (window.Office.context.mailbox && window.Office.context.mailbox.item.setSelectedDataAsync)
                return "1.2?";
            if (window.Office.context.mailbox && window.Office.context.mailbox.item.removeAttachmentAsync)
                return "1.1?";
            return "1.0?";
        }
        catch (e) {
    ...
    function ensureAppDiagnostics() {
        try {
            ...
            appInsights.trackEvent("diagError", { source: "Diagnostics.ensureAppDiagnostics", exception: e.toString(), message: e.message, stack: e.stack });
        }
    }
    ...
    function get() {
        if (!inGet) {
            ...
            try {
                ...
                appInsights.trackEvent("diagError", { source: "Diagnostics.get", exception: e.toString(), message: e.message, stack: e.stack });
            }
            inGet = false;
        }
    ...
    function set(field, value) {
        try {
            ...
            appInsights.trackEvent("diagError", { source: "Diagnostics.set", exception: e.toString(), message: e.message, stack: e.stack });
        }
    }
    function clear() { itemDiagnostics = null; }
    function ensureLastModified() {
        try {
            var client_1 = new XMLHttpRequest();
            // version.js is generated on build and is the true signal of the last modified time
            client_1.open("HEAD", mhaVersionScriptPath, true);
            client_1.onreadystatechange = function () {
                if (this.readyState === 2) {
                    lastUpdate = client_1.getResponseHeader("Last-Modified");
                }
            };
            client_1.send();
        }
        catch (e) {
            appInsights.trackEvent("diagError", { source: "Diagnostics.ensureLastModified", exception: e.toString(), message: e.message, stack: e.stack });
        }
    }
    ensureLastModified();
    return {
...
script.src = window.location.origin + '/vendor/stephenegriffin/MHA/aikey.js';
