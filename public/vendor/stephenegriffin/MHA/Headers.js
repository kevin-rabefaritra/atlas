...
var HeaderModel = (function (headers) {
    ...
    function toString() {
        ...
        if (summary.exists())
            ret.push(summary);
        if (receivedHeaders.exists())
            ret.push(receivedHeaders);
        if (forefrontAntiSpamReport.exists())
            ret.push(forefrontAntiSpamReport);
        if (antiSpamReport.exists())
            ret.push(antiSpamReport);
        if (otherHeaders.exists())
            ret.push(otherHeaders);
        return ret.join("\n\n");
    }
    if (headers) {
